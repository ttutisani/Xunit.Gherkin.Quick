...
public class SystemFeatureFileTests
{
    ...
    {
        var systemFeatureFile = new SystemFeatureFile(new DirectoryInfo(Environment.CurrentDirectory), new FileInfo(Path.Join(Environment.CurrentDirectory, "Features", "Addition_ForMultipleUseCases", "AddTwoNumbers.feature")));

        Assert.Equal("AddTwoNumbers.feature", systemFeatureFile.Name);
        Assert.Equal("./Features/Addition_ForMultipleUseCases/AddTwoNumbers.feature", systemFeatureFile.FullName);
    }
    ...
    public void Initialize_WhenFileNameIsOutsideOfRootDirectory_ThrowsException()
    {
        var exception = Assert.Throws<ArgumentException>(
            ...
            () => new SystemFeatureFile(
                ...
                new FileInfo(Path.Join(Environment.CurrentDirectory, "Features", "Addition_ForMultipleUseCases", "AddTwoNumbers.feature"))
            )
        );
    ...
    {
        var systemFeatureFile = new SystemFeatureFile(new DirectoryInfo(Environment.CurrentDirectory), new FileInfo(Environment.CurrentDirectory + "\\Features\\Addition_ForMultipleUseCases\\AddTwoNumbers.feature"));
